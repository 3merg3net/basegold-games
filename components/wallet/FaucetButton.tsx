...
export default function FaucetButton() {
  ...
  if (!mounted) {
    return (
      <button className="px-3 py-1 rounded bg-white/10 text-white/70">
        …
      </button>
    )
  }
  ...
  const busy = isLoading || wait.isLoading
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        onClick={onMint10k}
        disabled={busy}
        className="px-3 py-1 rounded bg-[#FFD700] text-black font-semibold hover:bg-[#FFE34F] border border-[#FFD700] shadow-md disabled:opacity-60"
      >
        {busy ? 'Minting…' : 'Mint 10,000 Chips'}
      </button>
      ...
      {error && (
        <span className="text-xs text-red-400">
          {(error as any)?.shortMessage || String(error)}
        </span>
      )}
    </div>
  )
