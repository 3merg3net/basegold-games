...
export default function HandYoureDealtArcadeMachine() {
  ...
  // ROOT WRAPPER — full overlay when fullscreenMobile
  return (
    <div
      className={[
      ...
      >
        ...
        >
          ...
          <div className="relative mx-auto w-full max-w-[520px] sm:max-w-[600px] aspect-[3/4]">
            ...
            {/* CARD WINDOW (center transparent area) – POSITIONS UNCHANGED */}
              ...
              {cards.map((card, i) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      'relative h-20 w-[3.2rem] sm:h-24 sm:w-[3.6rem] rounded-xl bg-slate-950 border border-yellow-400/70 shadow-[0_12px_24px_rgba(0,0,0,0.9)] flex items-center justify-center transition-transform duration-150',
                      spinning && !isHeld ? 'scale-95' : 'scale-100',
                      isHeld ? 'ring-2 ring-emerald-300' : '',
                    ].join(' ')}
                  >
                    {/* Inner realistic card face */}
                    <div
                      className={[
                        'relative h-[88%] w-[82%] rounded-[10px] border bg-white flex flex-col items-center justify-center',
                        card.isWild
                          ? 'border-amber-400 bg-gradient-to-b from-amber-50 via-yellow-50 to-amber-100'
                          : 'border-slate-300',
                      ].join(' ')}
                    >
                      {card.isWild ? (
                        <div className="flex flex-col items-center justify-center">
                          <span className="text-[11px] font-black tracking-[0.18em] text-yellow-600">
                            WILD
                          </span>
                          <span className="mt-0.5 text-[9px] text-amber-700/80">
                            Bonus Unlock
                          </span>
                        </div>
                      ) : (
                        <>
                          {/* corner rank/suit */}
                          <div className="absolute left-1 top-1 flex flex-col items-center text-[9px] font-semibold">
                            <span className={isRed ? 'text-red-500' : 'text-slate-900'}>
                          ...
                          <div className="absolute right-1 bottom-1 rotate-180 flex flex-col items-center text-[9px] font-semibold">
                            <span className={isRed ? 'text-red-500' : 'text-slate-900'}>
                              {rankToLabel(card.rank)}
                            </span>
                            <span className={isRed ? 'text-red-500' : 'text-slate-800'}>
            ...
            {/* DEAL / SPIN BUTTON INSIDE CABINET – POSITIONS UNCHANGED */}
            <button
              onClick={spin}
              disabled={!canSpin}
          ...
          <div className="rounded-[24px] border border-emerald-400/40 bg-gradient-to-b from-[#064e3b] via-[#022c22] to-black p-4 md:p-5 text-xs text-white space-y-4">
            ...
            <div>
              ...
              {credits <= 0 && (
                <div className="mt-1 text-[11px] text-rose-300">
                  You&apos;re out of demo credits. Top up from the arcade HUD
                  to keep spinning.
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-white/12 bg-black/40 p-3 space-y-2">
              ...
              <ul className="space-y-1 text-white/75 list-disc list-inside">
                ...
                <li>
                  Hands pay according to the paytable. Demo wallet tracks P&amp;L
